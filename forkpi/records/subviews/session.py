...
def logging_in(request):
    ...
    if user is not None:
        if user.is_superuser and user.is_active:
            ...
            messages.add_message(
                request, messages.SUCCESS, "Awesome! Your login was successful."
            )
            return redirect_to_name("keypairs")
...
def logout(request):
    if request.user.is_authenticated():
        ...
        messages.add_message(
            request, messages.ERROR, "If you're not logged in, how can you log out?"
        )
        return redirect_to_name("login")
...
def must_be_logged_in(request):
    messages.add_message(
        request, messages.ERROR, "You must be logged in to access that page."
    )
    return redirect_to_name("login")
