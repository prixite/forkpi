...
def keypairs_page(request):
    ...
    deactivated_keypairs = Keypair.objects.filter(is_active=False).order_by(
        "-last_edited_on"
    )
    ...
    return render(
        request,
        "keypairs.html",
        {"keypairs": keypairs, "deactivated_keypairs": deactivated_keypairs},
    )
...
def new_keypair(request):
    ...
    if not is_valid_name(name):
        messages.add_message(
            request, messages.ERROR, "Name must be unique and not blank."
        )
        is_error = True
    if pin and not is_valid_pin(pin):
        messages.add_message(
            request, messages.ERROR, "PIN must be at least 4 numeric characters."
        )
        is_error = True
...
def edit_keypair_pin(request):
    ...
    if is_valid_pin(pin):
        ...
        response = HttpResponse(
            "Invalid PIN: PIN must be at least 4 numeric characters."
        )
        response.status_code = 400
        return response
...
def edit_keypair_rfid(request):
    ...
    if rfid_uid == "" and not is_valid_keypair_rfid(kid):
        ...
        response = HttpResponse(
            "Error: A fingerprint template OR an RFID card must be entered."
        )
        response.status_code = 400
        return response
...
def authenticate_credential(request):
    ...
    try:
        ...
        response_text = {
            "pin": decrypt(keypair.pin),
            "rfid_uid": decrypt(keypair.rfid_uid),
            "fingerprint_template": keypair.fingerprint_template,
        }
