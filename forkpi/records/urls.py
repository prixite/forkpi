...
urlpatterns = patterns(
    ...
    url(
        r"^keypairs/edit/fingerprint$",
        edit_keypair_fingerprint,
        name="edit keypair fingerprint",
    ),
    url(r"^keypairs/delete$", delete_keypair, name="delete keypair"),
    url(
        r"^keypairs/toggleactive$", keypair_toggle_active, name="keypair toggle active"
    ),
    url(r"^keypairs/scan/rfid$", scan_rfid, name="scan rfid"),
    url(
        r"^keypairs/scan/fingerprint/3x$",
        scan_fingerprint_3x,
        name="scan fingerprint 3x",
    ),
    url(
        r"^keypairs/scan/fingerprint/wait$",
        wait_to_remove_finger,
        name="wait to remove finger",
    ),
    url(
        r"^keypairs/scan/fingerprint/1x$",
        scan_fingerprint_1x,
        name="scan fingerprint 1x",
    ),
    url(r"^keypairs/search", search_keypairs, name="search keypairs"),
    url(
        r"^keypairs/authenticate_credential$",
        authenticate_credential,
        name="authenticate credential",
    ),
    url(r"^keypairs/link_door$", link_door_to_keypair, name="link door to keypair"),
    url(
        r"^keypairs/unlink_door$",
        unlink_door_from_keypair,
        name="unlink door from keypair",
    ),
    ...
    url(
        r"^doors/unlink_keypair$",
        unlink_keypair_from_door,
        name="unlink keypair from door",
    ),
