...
class timespec(Structure):
    _fields_ = [("sec", time_t), ("nsec", c_long)]
...
CLOCK_MONOTONIC_RAW = (
    4  # Monotonic system-wide clock, not adjusted for frequency scaling.
)
CLOCK_REALTIME_COARSE = (
    5  # Identifier for system-wide realtime clock, updated only on ticks.
)
CLOCK_MONOTONIC_COARSE = 6  # Monotonic system-wide clock, updated only on ticks.
CLOCK_BOOTTIME = (
    7  # Monotonic system-wide clock that includes time spent in suspension.
)
...
timerfd_settime = syscall.lookup(
    c_int, "timerfd_settime", (c_int, c_int, POINTER(itimerspec), POINTER(itimerspec))
)
...
class Timer(syscall.SelfClosing):
    ...
    def start(self):
        ...
        if self._offset == 0 and self._interval == 0:
            raise ValueError(
                "timer will not fire because offset and interval are both zero"
            )
