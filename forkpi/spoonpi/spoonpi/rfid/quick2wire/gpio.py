...
class Pin(PinAPI):
    ...
    def __init__(
        self, bank, index, soc_pin_number, direction=In, interrupt=None, pull=None
    ):
        """Creates a pin
    ...
    def open(self):
        ...
        if self._direction == In:
            self._write(
                "edge", self._interrupt if self._interrupt is not None else "none"
            )
    ...
    def __str__(self):
        return "{type}({index})".format(type=self.__class__.__name__, index=self.index)
