...
bank_register_names = sorted(
    [s for s in globals().keys() if s.upper() == s], key=lambda s: globals()[s]
)
...
IOCONB = _banked_register(
    _BankB, IOCON
)  # Actually addresses the same register as IOCONA
IOCON_BOTH = IOCONA
...
register_names = sorted(
    [s for s in globals().keys() if s[-1] in ("A", "B") and s.upper() == s],
...
class PinBanks(object):
    ...
    def reset(
        self, interrupt_polarity=0, interrupt_open_drain=False, interrupt_mirror=True
    ):
        ...
        self.registers.reset(
            (interrupt_polarity << IOCON_INTPOL)
...
class PinBank(PinBankAPI):
    ...
    def __init__(self, chip, bank_id):
        ...
        self._register_cache = [
            None
        ] * BANK_SIZE  # self._register_cache[IOCON] is ignored
    ...
    def _read_register(self, register):
        self._register_cache[register] = self.chip.registers.read_banked_register(
            self._bank_id, register
        )

    def _set_register_bit(self, register, bit_index, new_value):
        self._register_cache[register] = _set_bit(
            self._register_cache[register], bit_index, new_value
        )
