...
def register_door(serial_num, door_name):
    ...
    c.execute(
        "SELECT id FROM records_door WHERE name='%s' AND serial != '%s'"
        % (door_name, serial_num)
    )
    result = c.fetchone()
    if result:
        ...
        if result:
            c.execute(
                "UPDATE records_door SET name='%s' WHERE id=%s" % (door_name, result[0])
            )
            conn.commit()
            print("Changed name from '%s' to '%s'!" % (result[1], door_name))
        else:
            c.execute(
                "INSERT INTO records_door(name, serial) VALUES ('%s', '%s')"
                % (door_name, serial_num)
            )
            conn.commit()
